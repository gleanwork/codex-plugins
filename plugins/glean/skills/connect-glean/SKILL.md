...
The Glean plugin ships skills (and supporting agents where the host supports
them) but **does not bundle an MCP server** — the skills call whatever Glean MCP
tools the host exposes. If no Glean MCP server is connected, the skills have
nothing to call. This skill connects one, or verifies and troubleshoots an
existing connection. Follow the section for the user's host.
